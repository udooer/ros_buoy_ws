...
class plot_hydrophone_data_node:
    ...
    def callback_2(self, data):
        ...
        M = len(abs_fft_ch1)/2+1
        self.f_max = self.fs/len(abs_fft_ch1)*M
        number = int(self.plot_length//data.delta_t)

        self.fft_matrix_ch1.append(abs_fft_ch1)
        self.fft_matrix_ch2.append(abs_fft_ch2)
        if(len(self.fft_matrix_ch1)>number):
            print(self.count_f)
            self.count_f+=1
            self.fft_matrix_ch1 = self.fft_matrix_ch1[-number:]
            self.fft_matrix_ch2 = self.fft_matrix_ch2[-number:]
    ...
    def animate(self, i):
        ...
        if(len(fft_matrix_ch1)!=0):
            self.ax[1][0].imshow(np.array(fft_matrix_ch1).T, origin="lower", extent=[0, self.plot_length, 0, self.f_max])
            self.ax[1][0].axis('auto')

            self.ax[1][1].imshow(np.array(fft_matrix_ch2).T, origin="lower", extent=[0, self.plot_length, 0, self.f_max])
...
def main():
    ...
    ani = FuncAnimation(pn.fig, pn.animate, interval=1000)
    plt.show()   
    rospy.spin()
